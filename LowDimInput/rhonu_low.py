...
path_plot = 'Plots_rhonu_low/'
path_data = 'Data_rhonu_low/'
...
plt.imshow(np.fabs(np.dot(v[:,0:1].T, v_theory.T)), interpolation = 'nearest', vmax = 1, vmin = -1, cmap='RdBu_r')
...
plt.imshow(np.dot(v[:,0:1].T, np.vstack([m, u]).T), interpolation = 'nearest', vmax = 1, vmin = -1, cmap='RdBu_r')
...
plt.imshow(np.fabs(np.dot(v[:,0:1].T, var.Normalize(np.random.normal(0, 1, (N,1))))), interpolation = 'nearest', vmax = 1, vmin = -1, cmap='RdBu_r')
